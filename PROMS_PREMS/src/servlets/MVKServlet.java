		...
		try {
			if (requestvalue != null) {
				...
				if (requestvalue[0].equals("createForm")) {
					String healthcareFacilityCareUnit = "apigw-dev"; // test
																		// clinic
					// String templateid =
					// "215edffa-538e-4508-b294-c46f69233c96";
					...
					String query = String
							.format("<urn:createForm xmlns:urn=\"urn:org:apigw:infrastructure:supportservices:forminteraction:MimeTypes:1\">"
									...
									+ "</urn:createForm>",
									URLEncoder
											.encode(healthcareFacilityCareUnit,
													charset),
									URLEncoder.encode(
											request.getParameterValues("templateID")[0],
											charset),
									URLEncoder.encode(
				 ...
				 */
					...
					System.out.println(connection.getResponseCode() + ": "
							+ connection.getResponseMessage());
					Map<String, List<String>> map1 = connection
							.getHeaderFields();
					for (Map.Entry<String, List<String>> entry : map1
							.entrySet()) {
						System.out.println("Key : " + entry.getKey()
								+ " , Value : " + entry.getValue());
					}
				 ...
				 * Completing a form
				 */
				else if (requestvalue[0].equals("completeForm")) {
					String form_id = request.getParameterValues("formID")[0];
					URL url = new URL(mvk + "/" + form_id + "/send");
					HttpURLConnection connection = (HttpURLConnection) url
							.openConnection();
					connection.setRequestProperty("Authorization",
							"Bearer cf70be6c-9dcd-11e1-8c70-12313c036113");
